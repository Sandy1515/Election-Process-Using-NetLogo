globals [NVoters]
patches-own [votes nextPref candidate winner loser listc]
...
to setup
  ...
  let initListOfList []
  repeat NCandidates [
    set initListOfList lput n-values NCandidates [0]
initListOfList
  ]

ask patches [
    set votes n-values NCandidates [0]
    set nextPref initListOfList
    set listc range NCandidates
  ]
  reset-ticks
end
...
to second_pref_vote
  let votes2 item candidate nextPref
  let pref2 random (NCandidates - 1)
  if pref2 >= candidate [
    set pref2 pref2 + 1
  ]
  set votes2 replace-item candidate nextPref votes2
end
...
to compute
  ...
  show (list "Min" vmin "Max" vmax)

end

to process
  repeat NCandidates - 2 [
    apply_nextPref
    compute
  ]
end
